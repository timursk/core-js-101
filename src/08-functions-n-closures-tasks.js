...
function getPolynom(...args) {
  ...
  return (x) => {
    ...
    args.forEach((num, idx) => {
      const pow = length - idx;
      result += num * (x ** pow);
    });
    return result;
  };
...
function logger(func, logFunc) {
  return (...val) => {
    ...
    let str = `${func.name}(${valStr.slice(1, valStr.length - 1)}) starts`;
    logFunc(str);
    const res = func(...val);
    str = `${func.name}(${valStr.slice(1, valStr.length - 1)}) ends`;
    logFunc(str);
    return res;
  };
}
